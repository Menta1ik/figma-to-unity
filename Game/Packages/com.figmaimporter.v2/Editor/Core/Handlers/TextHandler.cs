        ...
        public void Apply(FigmaNode node, FigmaElement target, FigmaHandlerContext context)
        {
            ...
            if (node.style != null)
            {
                ...
                if (context.FontMappings != null && node.style != null)
                {
                    ...
                    {
                        Debug.LogWarning($"<color=red>[FigmaImporter] Font not found for '{node.name}':</color> '{figmaFamily}' (Weight: {weight}, PostScript: {postScript}). " +
                                         $"Fallback used: {(context.GlobalFont != null ? context.GlobalFont.name : "None")}");
                    ...
                    {
                        Debug.Log($"<color=gray>[Diagnostic]</color> Text '{node.name}' hidden (Visible: {isVisible}, Opacity: {nodeOpacity})");
                    }
