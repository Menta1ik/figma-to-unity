...
- **Задача:** перед скачиванием группировать `ImageNodesToDownload` по `imageRef`, скачивать/импортировать спрайт один раз на уникальный `imageRef`, всем нодам с этим `imageRef` присваивать один и тот же `Sprite` asset. Чистая оптимизация внутри `ImageSyncService`, архитектуру не трогает.

### 2. Component Instances пересобираются с нуля вместо переиспользования префаба (дорого, архитектурно)
