    ...
    public class FigmaSyncManagerEditor : UnityEditor.Editor
    {
        ...
        private async void RunSync(FigmaSyncManager manager)
        {
            ...
            if (manager.UseLocalJson)
            {
                ...
                {
                    FigmaLog.Error($"[Figma v2.5.0] Файл не найден: {path}");
            ...
            try 
            {
                ...
                FigmaLog.Info("<color=green>[Figma v2.5.0]</color> Успешно!");
            }
            catch (System.Exception e)
            {
                FigmaLog.Error($"[Figma v2.5.0] Ошибка: {e.Message}");
        ...
        private void ClearChildren(Transform parent)
        {
            ...
            FigmaLog.Info("[Figma v2.5.0] Дочерние объекты очищены!");
        }
    }
}
